...
class MessageGroup extends \yii\db\ActiveRecord
{
    ...
    */
    ...
    public function getActiveGroup($userId)
    {
        ...
        ->where(['message_group.status'=>self::STATUS_ACTIVE])
        //->orderBy(['status'=>self::STATUS_ACTIVE])

        ->andWhere(['or', ['message_group.sender_id'=>$userId], ['message_group.receiver_id'=>$userId]])->all();
